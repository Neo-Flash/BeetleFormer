...
def set_style(context="paper", font_scale=1.5):
    ...
    Font sizes below are absolute (in points) so they are deterministic across
    figures regardless of the seaborn context; they are set generously so that
    axis numbers, labels and titles remain legible when the figure is scaled
    down to a single manuscript column.
    """
    sns.set_theme(context=context, style="ticks", font_scale=font_scale,
                  rc={
                      ...
                      # explicit, generous point sizes (override context scaling)
                      "font.size": 15,
                      "axes.labelsize": 16,
                      "axes.titlesize": 16,
                      "xtick.labelsize": 14,
...
def panel_label(ax, label, x=-0.10, y=1.05, size=16):
